...
def generate_single_question_from_source(self, source_data: Dict) -> Dict:
    ...
    try:
        ...
        # Find the corresponding ExpertQuestion
        try:
            expert_question = ExpertQuestion.objects.get(question_id=question_id)
        except ExpertQuestion.DoesNotExist:
            logger.warning(f"ExpertQuestion not found for question_id: {question_id}")
        ...
        if generated_question:
            # Create AIGeneratedQuestion record with proper mapping
            ai_question = AIGeneratedQuestion.objects.create(
                original_question_id=question_id,
                expert_question=expert_question,  # Direct FK reference
                domain=domain,
                question_type=question_type,
                ...
                generation_params={
                    ...
                    'temperature': 0.7,
                    'domain': domain,
                    'expert_question_id': expert_question.id if expert_question else None
                },
                generation_status='completed',
                quality_score=generated_question.get('confidence_score', 0.8),
                research_batch=f"csv_batch_{timezone.now().strftime('%Y%m%d')}"
            )
            
            logger.info(f"Successfully generated question for {question_id} -> AI Question ID: {ai_question.id}")
            return {
                'success': True,
